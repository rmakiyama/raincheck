...
import { type RaindropRecord, toBookmark } from "../src/raindrop/map.ts";
import {
  createRaindropSource,
  RaindropHttpError,
} from "../src/raindrop/source.ts";
...
describe("toBookmark", () => {
  it("maps every documented field", () => {
    expect(toBookmark(records[0]!)).toEqual({
      ...
      summary:
        "A deep dive into stable/unstable types and how the compiler reasons about them.",
...
describe("createRaindropSource", () => {
  ...
  const collect = async (
    src: ReturnType<typeof createRaindropSource>,
    limit?: number,
  ) => {
    const out = [];
    for await (const bookmark of src.fetch(
      limit === undefined ? {} : { limit },
    ))
      out.push(bookmark.id);
    return out;
  };
  ...
  it("does not count filtered records toward the limit", async () => {
    const { fetch } = stubFetch([
      { result: true, items: [records[1], records[0], records[2]] },
    ]);
  ...
  it("uses the given collection id", async () => {
    const { fetch, calls } = stubFetch([]);
    await collect(
      createRaindropSource({ token: "tok", fetch, collectionId: -1 }),
    );
