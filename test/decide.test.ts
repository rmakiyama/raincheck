...
const answers = (distance: JevAnswer, effect: JevAnswer): JevAnswers => ({
  distance,
  effect,
});
...
describe("decide", () => {
  it("reads the outcome table by the most likely level of distance and effect", () => {
    expect(
      decide(bookmark, res(answers(score(0, 0.1, 0.9), score(0.2, 0.7, 0.1))))
        .decision,
    ).toBe("helps");
    expect(
      decide(bookmark, res(answers(score(0, 0.6, 0.4), score(0.2, 0.7, 0.1))))
        .decision,
    ).toBe("related");
    expect(
      decide(bookmark, res(answers(score(0, 0.6, 0.4), score(0.1, 0.1, 0.8))))
        .decision,
    ).toBe("related");
    expect(
      decide(bookmark, res(answers(score(0, 0.1, 0.9), score(0.8, 0.2, 0))))
        .decision,
    ).toBe("skip");
    expect(
      decide(bookmark, res(answers(score(0.9, 0.1, 0), score(0.1, 0.1, 0.8))))
        .decision,
    ).toBe("skip");
  });
  ...
  it("does not let an even split promote an article: the mean would round up, the most likely level does not", () => {
    ...
    expect(
      decide(bookmark, res(answers(torn, score(0.3, 0.6, 0.1)))).decision,
    ).toBe("related");
    expect(
      decide(bookmark, res(answers(score(0, 0.5, 0.5), score(0.3, 0.6, 0.1))))
        .decision,
    ).toBe("related");
  });
  ...
  it("skips when a deciding answer is missing, not a score, or lacks a level probability", () => {
    expect(
      decide(bookmark, res({ distance: score(0, 0.1, 0.9) })).decision,
    ).toBe("skip");
    expect(
      decide(
        bookmark,
        res({
          ...answers(score(0, 0.1, 0.9), score(0, 0.1, 0.9)),
          effect: { type: "noul", noul: 1 },
        }),
      ).decision,
    ).toBe("skip");
    expect(
      decide(bookmark, res(answers(score(0, 0.1, 0.9), score(0.2, 0.8))))
        .decision,
    ).toBe("skip");
  });
...
describe("rank", () => {
  const v = (id: string, distance: JevAnswer, effect: JevAnswer): Verdict =>
    decide({ ...bookmark, id }, res(answers(distance, effect)));
  ...
  it("puts a verdict without levels last and does not mutate the input", () => {
    const consultedVerdict: Verdict = {
      bookmark: { ...bookmark, id: "x" },
      answers: {},
      decision: "consulted",
    };
    const input = [
      consultedVerdict,
      v("a", score(0.9, 0.1, 0), score(0.9, 0.1, 0)),
    ];
...
describe("consulted", () => {
  ...
  it("is true when a prompt contains the URL, ignoring scheme, www, trailing slash and fragment", () => {
    const w = work(
      "read https://docs.example.com/cookbooks/a/ and tell me",
      "unrelated prompt text here",
    );
    expect(consulted(w, at("https://docs.example.com/cookbooks/a"))).toBe(true);
    expect(
      consulted(w, at("http://www.docs.example.com/cookbooks/a/#setup")),
    ).toBe(true);
  });
  ...
  it("matches whole URLs pasted before punctuation", () => {
    expect(
      consulted(
        work("読んで: https://docs.example.com/cookbooks/a。あとで"),
        at("https://docs.example.com/cookbooks/a"),
      ),
    ).toBe(true);
    expect(
      consulted(
        work("see (https://docs.example.com/cookbooks/a), then"),
        at("https://docs.example.com/cookbooks/a"),
      ),
    ).toBe(true);
    expect(
      consulted(
        work("see https://docs.example.com/cookbooks/a."),
        at("https://docs.example.com/cookbooks/a"),
      ),
    ).toBe(true);
  });
  ...
  it("is false for a different page, a page under the bookmark, a longer host, or no URL at all", () => {
    ...
    expect(consulted(w, at("https://docs.example.com/cookbooks/b"))).toBe(
      false,
    );
    ...
    expect(
      consulted(
        work("read https://xdocs.example.com/cookbooks/a"),
        at("https://docs.example.com/cookbooks/a"),
      ),
    ).toBe(false);
    expect(
      consulted(
        work("read https://docs.example.com/cookbooks/abc"),
        at("https://docs.example.com/cookbooks/a"),
      ),
    ).toBe(false);
    expect(
