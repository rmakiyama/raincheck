...
describe("configure", () => {
    ...
    const io: ConfigureIO = {
      ...
      fetch: async (url, init) => {
        ...
        if (url.includes("typesafe"))
          return new Response("[]", { status: stub.typesafe ?? 200 });
        return new Response(
          JSON.stringify({ result: true, user: { fullName: "Ryo" } }),
          {
  ...
  it("requires a value when there is nothing to keep", async () => {
    await expect(configure(io({ answers: ["", ""] }).io)).rejects.toThrow(
  ...
  it("does not write anything when TypeSafe rejects the key", async () => {
    ...
    await expect(configure(i)).rejects.toThrow(
      "TypeSafe rejected the API key (401)",
    );
  ...
  it("does not write anything when Raindrop rejects the token", async () => {
    await expect(configure(io({ raindrop: 401 }).io)).rejects.toThrow(
