...
describe('createJevClient', () => {
  it('posts model, state and questions with the bearer key', async () => {
    ...
    const res = await jev.ask({ interests: 'x', article: { title: 't' } }, QUESTIONS)
  ...
  it('retries 429 and 529 with backoff, then succeeds', async () => {
    ...
    const fetch = async () => {
      ...
      return s ? new Response('slow down', { status: s }) : new Response(JSON.stringify(fixture))
    }
  ...
  it('gives up after maxRetries', async () => {
    ...
    const jev = createJevClient({ apiKey: 'k', model: 'm', fetch, maxRetries: 2, backoff: noBackoff })
  ...
  it('does not retry 401 or 422 and surfaces the body', async () => {
    ...
    const fetch = async () => {
      calls++
      return new Response('{"detail":"questions.depth.criteria: too short"}', { status: 422 })
    }
    const jev = createJevClient({ apiKey: 'k', model: 'm', fetch, backoff: noBackoff })
    await expect(jev.ask({}, QUESTIONS)).rejects.toThrow('422: {"detail":"questions.depth.criteria: too short"}')
    expect(calls).toBe(1)
  })
})
