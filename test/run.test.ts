  ...
  projects: [
    {
      ...
      sessions: 1,
      titles: [],
      prompts: ["Working on a Compose list screen"],
    },
  ],
};
...
const jevByTitle = (
  fail: number[] = [],
): JevAsker & { states: unknown[]; inFlight: number; peak: number } => {
  const asker = {
    states: [] as unknown[],
    inFlight: 0,
    peak: 0,
    async ask(state: unknown, questions: unknown): Promise<JevResponse> {
      ...
      const n = Number(
        /Article (\d+)/.exec(
          (state as { article: { title: string } }).article.title,
        )![1],
      );
      ...
      const answers: JevAnswers = {
        ...(fixture.answers as JevAnswers),
        distance: on(Math.max(0, 2 - n)),
        effect: on(1),
      };
      return {
        model: "jev-1.13.0",
...
describe("run", () => {
  it("judges every bookmark, ranks, and emits once", async () => {
    ...
    expect(sink.got.map((v) => v.bookmark.id)).toEqual([
      "raindrop:0",
      "raindrop:1",
      "raindrop:2",
    ]);
    expect(sink.got.map((v) => v.decision)).toEqual([
      "helps",
      "related",
      "skip",
    ]);
    expect(sink.got.map((v) => v.model)).toEqual([
      "jev-1.13.0",
      "jev-1.13.0",
      "jev-1.13.0",
    ]);
  ...
  it("does not judge a bookmark the prompts already refer to, and emits it as consulted", async () => {
    ...
    const work: RecentWork = {
      ...
      projects: [
        {
          ...recentWork.projects[0]!,
          prompts: ["look at https://example.com/seen/ first"],
        },
      ],
    };
    await run({
  ...
  it("builds state from recent work and the bookmark only", async () => {
    ...
    await run({
      bookmarks: source([
        { ...bookmark(0), note: "n", tags: ["t"], highlights: ["h"] },
      ]),
      interests,
      jev,
      sink: capture(),
  ...
  it("emits what was judged when the source fails mid-run", async () => {
    ...
    expect((result.sourceError as Error).message).toBe(
      "Raindrop HTTP 500: page 1",
    );
  });
...
describe("sinks", () => {
  ...
  it("stdout prints helps and related under their headings, capped by top per section, with a depth label", async () => {
    const w = writer();
    await createStdoutSink(w, { top: 1 }).emit([
      v(0, "helps"),
      v(1, "helps"),
      v(2, "related"),
      v(3, "skip"),
