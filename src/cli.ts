...
import { ConfigureError, configure, promptSecretTTY } from "./configure.ts";
import {
  createClaudeSessionsInterestSource,
  currentSessionId,
} from "./interests/claude-sessions.ts";
...
async function main(): Promise<number> {
  ...
  if (
    rest.length > 0 ||
    (command !== undefined && !["configure", "context"].includes(command))
  ) {
  ...
  if (values.current && values.days !== undefined) {
    throw new UsageError(
      "--current reads one whole session; --days does not apply",
    );
  }
  ...
  if (command === "context") {
    process.stdout.write(
      `${JSON.stringify(await interests.load(), null, 2)}\n`,
    );
    return 0;
  }
  ...
  const apiKey = requireSecret(
    "TYPESAFE_API_KEY",
    "typesafe_api_key",
    credentials.typesafeApiKey,
  );
  const token = requireSecret(
    "RAINDROP_TOKEN",
    "raindrop_token",
    credentials.raindropToken,
  );
  ...
  const result = await run({
    bookmarks: createRaindropSource({
      token,
  ...
  const count = (d: Verdict["decision"]) =>
    result.verdicts.filter((v) => v.decision === d).length;
  const consulted = count("consulted");
  const models =
    [...new Set(result.verdicts.map((v) => v.model).filter(Boolean))].join(
      ",",
    ) || "-";
  process.stderr.write(
  ...
  if (result.sourceError !== undefined) {
    process.stderr.write(
      `raincheck: source stopped early: ${describe(result.sourceError)}\n`,
    );
    return 1;
  }
...
function currentSessionSource(): InterestSource {
  ...
  if (!id)
    throw new UsageError(
      "--current only works inside a Claude Code session (CLAUDE_CODE_SESSION_ID is not set)",
    );
...
function requireSecret(
  ...
  if (!v)
    throw new UsageError(
      `${envName} is not set (env var, or "${fileKey}" in ${defaultCredentialsPath()})`,
    );
  return v;
}
