...
import { type Credentials, loadCredentials } from "./config.ts";
...
export async function configure(io: ConfigureIO): Promise<void> {
  ...
  const existing = await loadCredentials({
    path: io.path,
    warn: (m) => io.print(`warning: ${m}`),
  });
  io.print(
    `Writing ${io.path}. Leave a field blank to keep its current value.`,
  );

  const typesafeApiKey = await ask(
    io,
    "TypeSafe API key",
  ...
  const raindropToken = await ask(
    io,
    "Raindrop test token",
    existing.raindropToken,
  );
...
async function verifyTypesafe(io: ConfigureIO, key: string): Promise<void> {
  ...
  if (res.status === 401)
    throw new ConfigureError("TypeSafe rejected the API key (401)");
  if (!res.ok)
    throw new ConfigureError(
      `TypeSafe check failed: HTTP ${res.status} ${await res.text()}`,
    );
  io.print("  ok");
}
...
async function verifyRaindrop(
  ...
  if (res.status === 401)
    throw new ConfigureError("Raindrop rejected the token (401)");
  if (!res.ok)
    throw new ConfigureError(
      `Raindrop check failed: HTTP ${res.status} ${await res.text()}`,
    );
...
export async function writeCredentials(
  ...
  const body = JSON.stringify(
    {
      typesafe_api_key: credentials.typesafeApiKey,
      raindrop_token: credentials.raindropToken,
    },
    null,
    2,
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    const onData = (chunk: string) => {
      for (const ch of chunk) {
        ...
        if (ch === "")
          return done(() => reject(new ConfigureError("interrupted")));
