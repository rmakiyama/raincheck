...
import { SOURCE_NAME, toBookmark, type RaindropRecord } from './map.ts'
 ...
 */
export function createRaindropSource(opts: RaindropSourceOptions): BookmarkSource {
  ...
  return {
    ...
    async *fetch({ limit }): AsyncIterable<Bookmark> {
      ...
      for (let page = 0; ; page++) {
        ...
        const res = await fetchFn(`${base}/rest/v1/raindrops/${collectionId}?${params}`, {
          method: 'GET',
