...
export function resolveOptions(
  flags: Partial<Record<OptionName, string>>,
  config: Config,
): Options {
  const flag = (name: OptionName): number | undefined => {
    ...
    if (!OPTIONS[name].accepts(n))
      throw new OptionError(`--${name} must be ${OPTIONS[name].expected}`);
    return n;
  };
  return {
    ...
    concurrency:
      flag("concurrency") ?? config.concurrency ?? DEFAULTS.concurrency,
  };
}
...
export async function loadConfig(
  ...
  for (const key of Object.keys(CREDENTIAL_KEYS)) {
    if (parsed[key] !== undefined) {
      throw new Error(
        `${path}: "${key}" is a secret; move it to ${join(dirname(path), "credentials.json")}`,
      );
    }
  }
