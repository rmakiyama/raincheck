...
import { buildState, QUESTIONS } from "./questions.ts";
import type {
  Bookmark,
  BookmarkSource,
  InterestSource,
...
export async function run(opts: RunOptions): Promise<RunResult> {
  ...
  const judge = async (bookmark: Bookmark) => {
    ...
    try {
      const res = await opts.jev.ask(
        buildState(interests, bookmark),
        QUESTIONS,
      );
  ...
  const sourceError = await mapConcurrent(
    opts.bookmarks.fetch({ limit: opts.limit }),
    concurrency,
    judge,
  );
