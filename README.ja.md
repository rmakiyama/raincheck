...
URL があなたのプロンプトに出てくる記事は、すでに読んだものとみなして Jev に送らず、`consulted` として除外します。

## Questions

ブックマーク 1 件につき Jev へ 1 リクエストを送り、固定の 4 問と、最近の作業のプロジェクトごとに 1 問を答えさせます。質問は互いに独立で、記事同士を比較することはありません。
...
| `relevant_to::<プロジェクト名>` | noul | `relevant` と同じ問いを、そのプロジェクトだけに絞ったもの | 記録のみ。`relevant` が他プロジェクトに薄められていないかを見る |
| `distance`      | score | 記事は、いまの作業からどれだけ近いか（無関係 / 隣接 / まさにこれ） | 記録のみ。`relevant` + 閾値の置き換え候補 |
| `effect`        | score | 記事を読むと、いまの作業がどう変わるか（変わらない / 判断材料 / すぐ使える） | 記録のみ。同上 |
